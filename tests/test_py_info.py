...
@pytest.mark.skipif(python_implementation() == "PyPy", reason="setuptools distutils patching does not work")
...
if sys.version_info[:2] >= (3, 11):  # pragma: >=3.11 cover # pytest.skip doesn't support covdefaults
    ...
    @pytest.mark.skipif(os.name != "posix", reason="POSIX 3.11+ specific")
    def test_fallback_existent_system_executable(mocker: MockerFixture) -> None:
        ...
        current.executable = current.original_executable = os.path.join(current.prefix, "bin/python")  # noqa: PTH118
...
if sys.version_info[:2] == (3, 10):  # pragma: ==3.10 cover # pytest.skip doesn't support covdefaults
